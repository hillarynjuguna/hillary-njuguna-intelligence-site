...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const { name, email, score, readinessLevel, gaps } = data;
    ...
    if (postgresUrl) {
      try {
        await sql`
          ...
          VALUES (${name || 'Anonymous'}, ${email}, ${score}, ${readinessLevel}, ${JSON.stringify(gaps)})
    ...
    if (resend) {
      const gapsHtml = gaps.length > 0 
        ? `<ul>${gaps.map((g: string) => `<li><strong>Vulnerability Detected:</strong> ${g}</li>`).join('')}</ul>`
      ...
      try {
        await resend.emails.send({
          from: 'Oscillatory Fields <diagnostic@resend.dev>', // Use verified domain in prod
          to: [email],
