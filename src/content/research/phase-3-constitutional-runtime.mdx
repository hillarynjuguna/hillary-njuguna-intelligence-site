...
4. **τ/λ Delegation Rules:** The system maintains **Evaluation Autarky** (automated metric validation via $\Delta C_{joint}$) but explicitly rejects **Direction Autarky** (only the $\tau$-node, possessing $\chi=1$, may generate novel inquiry vectors).
   *Operational Proxy:* $\Delta C_{joint}$ is computed as $H(\lambda_{state}) + H(\mu_{audit}) - I(\lambda;\mu)$, utilizing bounded sampling windows and strict noise thresholds to ensure falsifiable signaling.
...
stateDiagram-v2
    ...
    Verification --> Resolution: $\Delta C_{joint}$ Threshold Passed
...
*   **Evidence Formats:** JSON-structured $\Delta C_{joint}$ telemetry coupled with human somatic recognition logs.
